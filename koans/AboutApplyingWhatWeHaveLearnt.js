...
describe("About Applying What We Have Learnt", function() {
  ...
  it("given I'm allergic to nuts and hate mushrooms, it should find a pizza I can eat (functional)", function () {
      ...
      productsICanEat = _(products).filter(function(pizza){
        let hasMush = _(pizza.ingredients).filter(function(ingred){
          if(ingred === "mushrooms") return ingred;
        });
        if(!pizza.containsNuts && !(hasMush.length > 0)){
          return pizza;
        }
      });
      expect(productsICanEat.length).toBe(1);
  ...
  it("should add all the natural numbers below 1000 that are multiples of 3 or 5 (functional)", function () {
    ...
    sum = _.range(0, 1000).reduce(function(acc, cval){
